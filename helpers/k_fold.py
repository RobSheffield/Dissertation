...
def _write_image_safely(image_path, image, attempts=2):
    """Write and validate an image before atomically exposing the final file."""
    stem, extension = os.path.splitext(image_path)
    temporary_path = f"{stem}.tmp{extension}"
    last_error = None
    ...
    for _ in range(attempts):
        try:
            ...
            if not cv2.imwrite(temporary_path, image):
                raise OSError("OpenCV reported that the image write failed")

            written_image = cv2.imread(temporary_path, cv2.IMREAD_UNCHANGED)
            if written_image is None or written_image.shape[:2] != image.shape[:2]:
                raise OSError("the written image could not be decoded correctly")
    ...
    if os.path.exists(temporary_path):
        os.remove(temporary_path)
    raise OSError(f"Failed to write a valid augmented image: {image_path}") from last_error
...
def augment_training_set_with_selected_transforms(
    train_img_dir,
    train_lbl_dir=None,
    include_horizontal_flip=False,
    include_vertical_flip=False,
    include_rot90=False,
    include_rot180=False,
    include_rot270=False,
):
    transforms = {
    ...
    if include_vertical_flip:
        enabled_transforms.append(('vflip', transforms['vflip']))
    if include_horizontal_flip:
        enabled_transforms.append(('hflip', transforms['hflip']))
    if include_rot90:
        enabled_transforms.append(('rot90', transforms['rot90']))
    if include_rot180:
        enabled_transforms.append(('rot180', transforms['rot180']))
    if include_rot270:
        enabled_transforms.append(('rot270', transforms['rot270']))
    ...
    for image_name in source_images:
        ...
        for suffix, transform_fn in enabled_transforms:
            ...
            aug_label_path = os.path.join(train_lbl_dir, f"{stem}_{suffix}.txt") if train_lbl_dir is not None else None

            _write_image_safely(aug_image_path, transformed_image)
            if aug_label_path is not None:
                _write_yolo_labels(aug_label_path, transformed_boxes)

def build_train_val_sets(folds_path, apply_training_augmentations=False, augmentation_options=None):
    ...
    for fold in folds:
        ...
        if augmentation_options is None and apply_training_augmentations:
        ...
        if augmentation_options:
            augment_training_set_with_selected_transforms(
                train_img,
                train_lbl,
                include_horizontal_flip=augmentation_options.get("horizontal_mirror", False),
                include_vertical_flip=augmentation_options.get("vertical_mirror", False),
                include_rot90=augmentation_options.get("rot90", False),
                include_rot180=augmentation_options.get("rot180", False),
                include_rot270=augmentation_options.get("rot270", False),
            )
...
def train_all(
    ...
    one_shot=False,
    weights="yolo11n.pt",
    group_outputs_by_fold=False,
):
    folds_path = _resolve_path(folds_path)
    model_dir = _resolve_path(model_dir)
    folds = sorted([f for f in os.listdir(folds_path) if f.startswith("fold_")])
    effective_batch_size = min(int(batch_size), 8) if int(img_size) >= 1280 else int(batch_size)
    ...
    for fold in folds if not one_shot else folds[:1]:
        ...
        local_weights = _resolve_path(weights)
        preweights = local_weights if os.path.isfile(local_weights) else weights
        model_info_json = {
            "name": fold,
            "model": preweights,
        ...
        fold_model_dir = os.path.join(model_dir, fold)
        if group_outputs_by_fold:
            fold_model_dir = os.path.join(fold_model_dir, "train")
...
def mAP_on_test_set(test_dir, model_dir, imgsz=640, validation_output_root=None):
    ...
    if os.path.isfile(model_dir) and model_dir.lower().endswith('.pt'):
        model_paths.append((os.path.splitext(os.path.basename(model_dir))[0], model_dir))
        ...
        for item in sorted(os.listdir(model_dir)):
            ...
            grouped_best_pt = os.path.join(fold_path, "train", "weights", "best.pt")
            if os.path.isfile(best_pt):
                model_paths.append((item, best_pt))
            elif os.path.isfile(grouped_best_pt):
                model_paths.append((item, grouped_best_pt))
    else:
    ...
    for fold_name, model_path in model_paths:
        print(f"Evaluating on test set: {model_path}")
        model = YOLO(model_path)

        validation_args = {}
        if validation_output_root is not None:
            fold_output_dir = os.path.join(_resolve_path(validation_output_root), fold_name)
            validation_dir = os.path.join(fold_output_dir, "val")
