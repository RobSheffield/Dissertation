...
from ui_tabs.sadl_tab import SADLTab
from ui_tabs.sadl_expansion_tab import SADLExpansionTab
from ui_tabs.kfold_tab import KFoldTab
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.sadl_tab = SADLTab()
        self.sadl_expansion_tab = SADLExpansionTab()
        self.kfold_tab = KFoldTab()

        self.testing_experiments_tab = QWidget()
        testing_experiments_layout = QVBoxLayout(self.testing_experiments_tab)
        self.testing_experiments_tabs = QTabWidget()
        self.testing_experiments_tabs.addTab(self.kfold_tab, "K-Folds Testing")
        self.testing_experiments_tabs.addTab(self.testing_tab, "Metamorphic Testing")
        self.testing_experiments_tabs.addTab(self.sadl_tab, "SADL")
        self.testing_experiments_tabs.addTab(self.sadl_expansion_tab, "SADL Expansion")
        testing_experiments_layout.addWidget(self.testing_experiments_tabs)
        ...
        # Qt uses ampersands to mark keyboard shortcuts; doubling it displays one.
        self.tabs.addTab(self.testing_experiments_tab, "Testing && Experiments")
    ...
    def resizeEvent(self, event):
        ...
        new_columns = max(1, self.width() // 200)  
    ...
    def closeEvent(self, event):
        ...
        if hasattr(self.sadl_tab, 'worker') and self.sadl_tab.worker is not None and self.sadl_tab.worker.isRunning():
            self.sadl_tab.worker.terminate()
            self.sadl_tab.worker.wait()
        if hasattr(self.sadl_expansion_tab, 'worker') and self.sadl_expansion_tab.worker is not None and self.sadl_expansion_tab.worker.isRunning():
            self.sadl_expansion_tab.worker.terminate()
            self.sadl_expansion_tab.worker.wait()
        if hasattr(self.kfold_tab, 'worker') and self.kfold_tab.worker is not None and self.kfold_tab.worker.isRunning():
            self.kfold_tab.worker.terminate()
            self.kfold_tab.worker.wait()
        event.accept()
