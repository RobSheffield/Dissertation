...
try:
	from . import DSA, LSA, sadl_helpers
...
def _augment_train_dir(train_dir, horizontal_mirror=True, vertical_mirror=False, rotate_90=False, rotate_180=False, rotate_270=False):
	if not any([horizontal_mirror, vertical_mirror, rotate_90, rotate_180, rotate_270]):
		return train_dir, None

	temp_dir = TemporaryDirectory(prefix="sadl_across_")
	temp_train_dir = Path(temp_dir.name) / "train"
	shutil.copytree(train_dir, temp_train_dir)
	...
	return temp_train_dir, temp_dir
...
def run_across(
	...
	with TemporaryDirectory(prefix="sadl_across_work_") as temp_work_dir:
		temp_folds_root = Path(temp_work_dir) / "folds"
		shutil.copytree(folds_root, temp_folds_root)

		for fold in folds:
			model_path = models_root / fold / "weights" / "best.pt"
			train_dir = temp_folds_root / fold / "images" / "train"
			val_dir = temp_folds_root / fold / "images" / "val"

			if not model_path.is_file():
				print(f"Skipping {fold}: missing model at {model_path}")
				continue
			if not train_dir.is_dir() or not val_dir.is_dir():
				print(f"Skipping {fold}: missing train/val dirs under {temp_folds_root / fold}")
				continue

			print(f"Processing {fold}")
			model = _load_model(str(model_path))
			augmented_train_dir, temp_dir = _augment_train_dir(
				str(train_dir),
